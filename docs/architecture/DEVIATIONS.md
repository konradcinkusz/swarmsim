...
| P5 (JWT / asymmetric signing), narrowed | `SwarmApi.Api` supports JWT bearer auth (RS256, JWKS) against an external `authservice` instance, but only gates `POST /api/missions`; `GET` endpoints and the dashboard stay open even when Enforced, and the dashboard has no login UI. | M3 (this phase); narrowed 2026-09-21 | See [ADR-0005](../adr/0005-mcp-server-and-bearer-auth.md). The write an MCP-driven Agent invokes is now attributable; reads and the dashboard are not yet. Trigger: a login flow for the dashboard, or a requirement to distinguish *readers*, not just mission dispatch. |
